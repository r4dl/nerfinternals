...
class aNeRFField(Field):
    ...
    def get_activation_in_layer(self, ray_samples: RaySamples, layer: int, app_act: Optional[bool] = True):
        """
        return activation from the pMLP in layer
        """
        if self.use_integrated_encoding:
            gaussian_samples = ray_samples.frustums.get_gaussian_blob()
            if self.spatial_distortion is not None:
            ...
            if self.spatial_distortion is not None:
                positions = self.spatial_distortion(positions)
            encoded_xyz = self.position_encoding(positions)
        return self.mlp_base.get_activation_in_layer(encoded_xyz, layer)