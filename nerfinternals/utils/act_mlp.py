"""
Multi Layer Perceptron that yields activation.
"""
...
class aMLP(FieldComponent):
    ...
    def get_activation_in_layer(self, in_tensor: Tensor,
        ...
        Args:
            in_tensor: Network input
            layer_idx: layer in which to observe the activation
            apply_activation: whether to apply the activation before returning
        Returns:
            MLP network output
        """
        # check if in bounds
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = layer(x)
            if self.activation is not None and i < len(self.layers) - 1:
                if i == layer_idx and not apply_activation:
                    pass
                else:
                    x = self.activation(x)
            if i == layer_idx:
